...
export class OpenAiResumeRewriteProvider implements ResumeRewriteProvider {
  ...
  async mapRequirements(input: ResumeRewriteInput): Promise<RequirementMapping[]> {
    const systemPrompt = await loadPrompt('map_requirements.md');
    return this.llmClient.completeJson<RequirementMapping[]>({
      systemPrompt,
      userPrompt: renderPrompt('Map these requirements to supported canonical evidence.', input),
    });
  }
